...
public class SimulateController  implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        Process.processes.sort(new ArrivalTimeComparator());
        SimulateController.processors = new LinkedList<>();
        TimeListSynchronization.canThreadTakeReadTimeVariable.add(true);  // Index 0, Global Queue's Thread.
        TimeListSynchronization.canThreadTakeReadTimeVariable.add(true);  // IO Queue Thread.
        for(int i = 0; i < SimulateController.numberOfProcessors; i++) {
            SimulateController.processors.add(new Processor());
            TimeListSynchronization.canThreadTakeReadTimeVariable.add(true);
        }
    ...
    public void simulateAlgorithm(){
            ...
            public void run() {
                while(true){
                    ...
                    for(int i = 0;i < numberOfThreads; i++)
                        if(TimeListSynchronization.canThreadTakeReadTimeVariable.get(i)){
                            allCPUsDone = false;
                            break;
                        }
                    if(allCPUsDone){
                        ...
                        for(int i = 0;i < numberOfThreads; i++)
                            TimeListSynchronization.canThreadTakeReadTimeVariable.set(i, true);
                    }
        ...
        Thread globalQueueThread = new Thread(new Runnable() {
            ...
            public void run() {
                while(true) {
                    ...
                    if(TimeListSynchronization.canThreadTakeReadTimeVariable.get(0)){
                        try {
                            Process front = null;
                            ...
                            TimeListSynchronization.canThreadTakeReadTimeVariable.set(0, false);
                        }catch (InterruptedException e){
                            e.printStackTrace();
                        }
                    }
        ...
        for(int i = 0 ; i < FinishedProcesessSynch.finishedProcesses.size() ; i++){
            Process process = FinishedProcesessSynch.finishedProcesses.get(i);
    ...
    public void runSimulation(ActionEvent event){
        ...
        if(selectedAlgorithm == null){
            ...
            while(FinishedProcesessSynch.finishedProcesses.peek() != null)
                Process.processes.add(FinishedProcesessSynch.finishedProcesses.poll());
            Process.processes.sort(new ArrivalTimeComparator());
